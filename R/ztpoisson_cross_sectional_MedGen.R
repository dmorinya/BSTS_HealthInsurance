...
data_poiss <- read_csv("/home/dmorina/Insync/dmorina@ub.edu/OneDrive Biz/Projectes/2022/0052022. MAPFRE/Original MAPFRE Data/MAPFRE_Poisson_general_medicine_data.csv")
...
#### zero-truncated Poisson regression global to estimate zeros in the data (and their distribution)
mod_glob_year <- vglm(Acts~Sex+AgeCAT+factor(Year), family = pospoisson(), data = global); summary(mod_glob_year)
...
tab_model(mod_glob_year2019)
tab_model(mod_glob_year2020)
tab_model(mod_glob_year2021)
tab_model(mod_glob_year2022)
...
table(global$Acts, global$Year)
colSums(table(global$Acts, global$Year))
